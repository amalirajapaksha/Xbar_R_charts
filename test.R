  ...
  "),
    ...
    ",
      ...
      <span style='color:#00C853; font-weight:bold;'>
        ✔ Out-of-control points are <u>highlighted</u> in the table below.
    ...
    ")
                 ...
                 h4("Phase I Data with R & X̄ Statistics"),
...
server <- function(input, output, session) {
  ...
  # Phase I data table with statistics
  combined_data <- reactive({
    df <- original_data()
    numeric_cols <- df %>% select(where(is.numeric)) %>% select(-Subgroup)
    
    # R and Xbar
    R_vals <- apply(numeric_cols, 1, function(x) max(x) - min(x))
    Xbar_vals <- apply(numeric_cols, 1, mean)
    ...
    R_OOC <- R_vals > s_r$UCL | R_vals < s_r$LCL
    if(!is.null(s_x)){
      Xbar_OOC <- Xbar_vals > s_x$Xbar_UCL | Xbar_vals < s_x$Xbar_LCL
    } else {
      Xbar_OOC <- rep(FALSE, nrow(df))
    }
    ...
    df %>%
      mutate(
        R = R_vals,
        Xbar = Xbar_vals,
        R_OutOfControl = R_OOC,
        Xbar_OutOfControl = Xbar_OOC,
        OutOfControl = R_OOC | Xbar_OOC
      )
  })
